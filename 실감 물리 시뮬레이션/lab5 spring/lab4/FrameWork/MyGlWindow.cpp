	...
	//==========================================================================
{
	...
	pickedPosition = cyclone::Vector3(0, 0, 0);
...
//==========================================================================
{
   ...
   //Draw axises
	...
	//draw shadow
	setupShadows();
	if (mover_connection) mover_connection->draw(1);
	unsetupShadows();
	...
	//draw objects
	glPushMatrix();
	if (mover_connection) mover_connection->draw(0);
	glPopMatrix();
...
//==========================================================================
{
	...
	case FL_PUSH:
	{
		...
		if (m_pressedMouseButton == 1) {
			...
			if (selected >= 0) {
				...
				mover_connection->m_movers[selected]->m_particle->getPosition(&pickedPosition);
			}
		}
	}
	damage(1);
	...
	case FL_RELEASE:
	{
		if (selected >= 0) {
			...
			mover_connection->m_movers[selected]->m_particle->getPosition(&mover_pos);
			cyclone::Vector3 new_vel = mover_pos - pickedPosition;
			mover_connection->m_movers[selected]->m_particle->setVelocity(new_vel);
	...
	case FL_DRAG: // if the user drags the mouse
	{
		...
		if (m_pressedMouseButton == 1) {
			if (selected >= 0) {
				...
				mover_connection->m_movers[selected]->m_particle->setPosition(cyclone::Vector3(rx, ry, rz));
				damage(1);
			} 
			else
