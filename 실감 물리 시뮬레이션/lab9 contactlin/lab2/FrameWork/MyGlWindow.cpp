...
//==========================================================================
{
	...
	mover_connection = new MoverConnection();

	m_resolver = new cyclone::ParticleContactResolver(0);
	
	m_contactGenerators.push_back(new cyclone::MyGroundContact());
	for each(Mover* m in mover_connection->m_movers)
		static_cast<cyclone::MyGroundContact*>(m_contactGenerators[0])->
		init(m->particle, 1.0);
	
	m_contactGenerators.push_back(new cyclone::MyGroundContact());
		static_cast<cyclone::ParticleCollision*>(m_contactGenerators[1])->
			particle[0] = mover_connection->m_movers[0]->particle;
		static_cast<cyclone::ParticleCollision*>(m_contactGenerators[1])->
			particle[1] = mover_connection->m_movers[1]->particle;
...
void MyGlWindow::update() 
{
	...
	mover_connection->update(duration);
	for(int i = 0; i < m_contactGenerators.size(); i++) {
		unsigned used = m_contactGenerators[i]->addContact(nextContact, limit);
		limit -= used;
		nextContact += used;
		if (limit <= 0)
			break;
...
//==========================================================================
{
  ...
  case FL_PUSH:
	  ...
	  if (m_pressedMouseButton == 1) {
		  doPick();
		  if (selected >= 0) {
			  std::cout << "Picked" << selected << std::endl;

			  mover_connection->m_movers[selected]->particle->
				  getPosition(&pickedPosition);
		  }
	  }
	  }
	  damage(1);
  ...
  case FL_RELEASE:
	  if (selected >= 0) {
		  cyclone::Vector3 mover_pos;
		  mover_connection->m_movers[selected]->particle->getPosition(&mover_pos);
		  cyclone::Vector3 new_vel = mover_pos - pickedPosition;
		  mover_connection->m_movers[selected]->particle->setVelocity(new_vel);
	  ...
	  if (m_pressedMouseButton == 1) {
		  if (selected >= 0) {
			  ...
			  mousePoleGo(r1x, r1y, r1z, r2x, r2y, r2z,
				  static_cast<double>(mover_pos.x),
				  static_cast<double>(mover_pos.y),
				  static_cast<double>(mover_pos.z),
				  rx, ry, rz,
				  (Fl::event_state() & FL_CTRL) != 0);
			  mover_connection->m_movers[selected]->particle->setPosition(cyclone::Vector3(rx, ry, rz));
			  damage(1);
		  }
		  else
