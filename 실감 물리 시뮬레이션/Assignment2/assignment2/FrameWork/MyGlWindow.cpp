...
static double DEFAULT_VIEW_POINT[3] = {60, 60, 0};
...
//==========================================================================
{
	...
	normal_fireworks_timer = 0.f;
	big_fireworks_timer = 0.f;
}
...
void MyGlWindow::update() 
{
	...
	normal_fireworks_timer += duration;
	if (normal_fireworks_timer >= 3) {
		normal_fireworks_timer = 0;
		Vector3 random_color = crandom.randomVector(Vector3(0, 0, 0), Vector3(1, 1, 1));
		for (int i = 0; i < 8; i++) {
			for (int j = 0; j < 8; j++) {
				double random_age = crandom.randomReal(1, 3);
				fire = fireworks.create();
				fire->setRule(&fireworks.rules[0]);
				fire->setParameter(1, random_age,
					Vector3(-100 + 24 * i, 2, -100 + 24 * j),
					Vector3(0, 9.8 * random_age, 0), random_color, 0.99f);
			}
		}
	}

	big_fireworks_timer += duration;
	if (big_fireworks_timer >= 10) {
		big_fireworks_timer = 0;
		Vector3 random_color = crandom.randomVector(Vector3(0, 0, 0), Vector3(1, 1, 1));
		fire = fireworks.create();
		fire->setRule(&fireworks.rules[1]);
		fire->setParameter(3, 4, Vector3(0, 3, 0), Vector3(0, 9.8 * 4, 0),
			random_color, 0.99f);
	}
}
