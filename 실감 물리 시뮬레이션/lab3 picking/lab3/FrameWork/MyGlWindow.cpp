...
//==========================================================================
{
	...
	case FL_PUSH:
	{
		...
		if (m_pressedMouseButton == 1) {
			...
			if (selected >= 0) {
				...
				m_mover->m_particle->getPosition(&pickedPosition);
			}
		}
	}
	damage(1);
	...
	case FL_RELEASE:
	{
		if (selected >= 0) {
			...
			m_mover->m_particle->getPosition(&mover_pos);
			cyclone::Vector3 new_vel = mover_pos - pickedPosition;
			m_mover->m_particle->setVelocity(new_vel);
	...
	case FL_DRAG: // if the user drags the mouse
	{
		...
		if (m_pressedMouseButton == 1) {
			if (selected >= 0) {
				...
				m_mover->m_particle->setPosition(cyclone::Vector3(rx, ry, rz));
				damage(1);
			} 
			else
