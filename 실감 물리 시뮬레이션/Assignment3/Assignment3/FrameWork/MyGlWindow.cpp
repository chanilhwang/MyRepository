	...
	//==========================================================================
{
	...
	//particle world
	m_world = new cyclone::ParticleWorld(38, 76);

	//bridge
	m_bridge = new MyBridge(m_world);
...
//==========================================================================
{
	...
	case FL_DRAG: // if the user drags the mouse
	{
		...
		if (selected >= 0 && m_pressedMouseButton == 1) {
			...
			Mover* selected_mover =  m_bridge->getMovers()[selected];
			cyclone::Vector3 moverpos = selected_mover->m_particle->getPosition();
			mousePoleGo(r1x, r1y, r1z, r2x, r2y, r2z,
				static_cast<double>(moverpos.x),
				static_cast<double>(moverpos.y),
				static_cast<double>(moverpos.z),
				rx, ry, rz,
				(Fl::event_state() & FL_CTRL) != 0);

			selected_mover->m_particle->setVelocity(cyclone::Vector3(rx, ry, rz) - moverpos);
